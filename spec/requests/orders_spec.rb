...
RSpec.describe "OrdersControllers", type: :request do
  describe "get orders_path" do
    it "renders the order index view" do
      order = FactoryBot.create_list(:order, 10)
      get orders_path
  ...
  describe "post order_path with valid data" do
    it "saves a new order entry and redirects to the show path for the entry" do
  ...
  describe "delete an order record" do
    it "deletes an order record" do
      order = FactoryBot.create(:order)
      order.destroy
      expect(get orders_path(id: 5000))
    end
  end
